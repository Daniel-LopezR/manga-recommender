...
const OptionsGenerator: React.FC<{
  ...
  return (
    <div className="flex flex-col w-2/4">
      ...
      <div id={`menuOptions-${title}`} className={`flex flex-wrap flex-row justify-around gap-2 ${(title === "Genres") ? "overflow-y-scroll h-24 overflow-x-hidden" : ""}`}>
        {props.dataFS &&
          props.dataFS.map((item) => {
            return (
              <div className="aux p-2 text-center border rounded-3xl transition cursor-pointer" key={`${item.id}-label`} onClick={checkOption}>
                <div className="base peer inline-block" id={`${item.id}-${title}`} key={`${item.id}-name`}>{item.name}</div>
